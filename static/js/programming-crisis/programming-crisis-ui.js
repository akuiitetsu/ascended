export class ProgrammingCrisisUI {
    ...
    renderTwoColumnLayout() {
        return `
            <div class="main-game-layout grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                <!-- Game Grid Column - Full width on mobile, left column on desktop -->
                <div class="debug-grid-column order-2 lg:order-1">
                    ${this.renderGameContainer()}
                </div>
                
                <!-- Controls Column - Full width on mobile, right column on desktop -->
                <div class="controls-column space-y-4 lg:space-y-6 order-1 lg:order-2">
    ...
    renderGameContainer() {
        return `
            <div class="game-container bg-gray-800 rounded-lg p-3 lg:p-4">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 lg:mb-4 gap-2">
                    <h3 class="text-lg lg:text-xl font-bold text-white">🎮 Debug Grid - Level ${this.room.currentLevel}</h3>
                ...
                <div class="debug-status-panel grid grid-cols-2 sm:grid-cols-4 gap-1 lg:gap-2 mb-3 lg:mb-4 p-2 lg:p-3 bg-gray-900 rounded-lg border border-gray-600">
                    <div class="status-metric text-center">
                        <div class="flex items-center justify-center mb-1">
                            <i class="bi bi-heart text-green-400 text-sm lg:text-lg mr-1"></i>
                            <span class="text-xs text-green-200 font-medium hidden sm:inline">Health</span>
                        </div>
                        <div id="player-health" class="text-sm lg:text-lg font-bold text-green-100">${this.room.player.health}</div>
                    </div>
                    <div class="status-metric text-center">
                        <div class="flex items-center justify-center mb-1">
                            <i class="bi bi-lightning text-blue-400 text-sm lg:text-lg mr-1"></i>
                            <span class="text-xs text-blue-200 font-medium hidden sm:inline">Energy</span>
                        </div>
                        <div id="player-energy" class="text-sm lg:text-lg font-bold text-blue-100">${this.room.player.energy}</div>
                    </div>
                    <div class="status-metric text-center">
                        <div class="flex items-center justify-center mb-1">
                            <i class="bi bi-bug text-purple-400 text-sm lg:text-lg mr-1"></i>
                        ...
                        <div id="bugs-defeated" class="text-sm lg:text-lg font-bold text-purple-100">${this.room.bugsDefeated}</div>
                    </div>
                    <div class="status-metric text-center">
                        <div class="flex items-center justify-center mb-1">
                            <i class="bi bi-layers text-yellow-400 text-sm lg:text-lg mr-1"></i>
                            <span class="text-xs text-yellow-200 font-medium hidden sm:inline">Level</span>
                        </div>
                        <div id="current-level" class="text-sm lg:text-lg font-bold text-yellow-100">${this.room.currentLevel}/${this.room.maxLevel}</div>
                    </div>
                </div>
                
                <!-- Game Grid - Responsive container -->
                <div class="grid-container flex justify-center mb-3 lg:mb-4 overflow-x-auto">
                ...
                <div class="grid-legend text-xs lg:text-sm text-gray-400 text-center mb-3 lg:mb-4">
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-1 lg:gap-2">
                        <span>🤖 = You</span>
                        <span>🐛 = Bug</span>
                        <span>🧱 = Wall</span>
                        <span>🔥 = Firewall</span>
                ...
                <div class="execution-queue bg-gray-900 p-2 lg:p-3 rounded border border-gray-700 mb-3 lg:mb-4">
                    <h5 class="font-bold text-blue-400 mb-2 flex items-center text-sm lg:text-base">
                        <i class="bi bi-gear-fill mr-2"></i>
                        Execution Queue
                    </h5>
                    <div id="queue-display" class="text-xs lg:text-sm text-gray-300 max-h-16 lg:max-h-20 overflow-y-auto">
                        <div class="text-gray-500">No commands queued</div>
                    </div>
                </div>
                
                <!-- Level Editor Button - Mobile responsive -->
                <div class="level-editor-section text-center pt-3 lg:pt-4 border-t border-gray-700">
                    <button id="open-level-editor" class="bg-blue-600 hover:bg-blue-700 px-3 lg:px-4 py-2 text-xs lg:text-sm rounded-lg transition-colors border border-blue-500">
                        <i class="bi bi-pencil-square mr-1 lg:mr-2"></i>
                        Level Editor
                    </button>
                    <p class="text-xs text-gray-400 mt-2 hidden sm:block">Create custom levels and challenges</p>
                </div>
            </div>
        `;
    }
    ...
    renderCodeEditor() {
        return `
            <div class="code-editor bg-gray-800 rounded-lg p-3 lg:p-4">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-2">
                    <h4 class="text-base lg:text-lg font-bold text-white">💻 Code Editor</h4>
                    <div class="code-controls flex gap-1 lg:gap-2 w-full sm:w-auto">
                        <button id="step-debug" class="bg-purple-600 hover:bg-purple-500 px-2 lg:px-3 py-1 text-xs lg:text-sm rounded transition-colors flex-1 sm:flex-none">
                            <i class="bi bi-skip-forward"></i> <span class="hidden sm:inline">Step</span>
                        </button>
                        <button id="execute-code" class="bg-green-600 hover:bg-green-500 px-2 lg:px-3 py-1 text-xs lg:text-sm rounded transition-colors flex-1 sm:flex-none">
                            <i class="bi bi-play-fill"></i> <span class="hidden sm:inline">Run</span>
                        </button>
                        <button id="stop-execution" class="bg-red-600 hover:bg-red-500 px-2 lg:px-3 py-1 text-xs lg:text-sm rounded transition-colors flex-1 sm:flex-none" disabled>
                ...
                <div class="mb-3">
                    <textarea id="code-input" 
                             class="w-full h-48 lg:h-64 bg-black text-green-400 font-mono p-2 lg:p-3 rounded border border-gray-500 text-xs lg:text-sm resize-none"
...
#         wait()"></textarea>
                ...
                <div class="code-actions flex flex-wrap gap-2 mb-3">
                    <button id="clear-code" class="bg-gray-600 hover:bg-gray-500 px-3 lg:px-4 py-2 text-xs lg:text-sm rounded transition-colors">
                        <i class="bi bi-trash"></i> Clear
                    </button>
                    <button class="bg-yellow-600 hover:bg-yellow-500 px-3 lg:px-4 py-2 text-xs lg:text-sm rounded transition-colors" onclick="this.nextSibling.nextSibling.classList.toggle('hidden')">
    ...
    renderCommandReference() {
        return `
            <div class="command-reference bg-gray-800 rounded-lg p-3 lg:p-4">
                <h4 class="text-base lg:text-lg font-bold text-white mb-3">📋 Command Reference</h4>
                
                <div class="commands-content space-y-3 lg:space-y-4 max-h-48 lg:max-h-64 overflow-y-auto">
                    <!-- Basic Commands -->
                    <div class="command-category">
                        <div class="font-bold text-blue-400 mb-2 text-sm lg:text-base">Basic Commands:</div>
                        <div class="command-list text-xs lg:text-sm text-gray-300 space-y-1 ml-2">
                    ...
                    <!-- Conditionals -->
                    <div class="command-category">
                        <div class="font-bold text-orange-400 mb-2 text-sm lg:text-base">Conditionals (Python syntax):</div>
                        <div class="command-list text-xs lg:text-sm text-gray-300 space-y-1 ml-2">
                    ...
                    <!-- Condition Functions -->
                    <div class="command-category">
                        <div class="font-bold text-green-400 mb-2 text-sm lg:text-base">Condition Functions:</div>
                        <div class="command-list text-xs lg:text-sm text-gray-300 space-y-1 ml-2">
                    ...
                    <!-- Loops -->
                    <div class="command-category">
                        <div class="font-bold text-pink-400 mb-2 text-sm lg:text-base">Loops (Python syntax):</div>
                        <div class="command-list text-xs lg:text-sm text-gray-300 space-y-1 ml-2">
                ...
                <!-- Inventory Section -->
                <div class="inventory-section mt-3 lg:mt-4 pt-3 lg:pt-4 border-t border-gray-600">
                    <h5 class="font-bold text-purple-400 mb-2 flex items-center text-sm lg:text-base">
                        <i class="bi bi-backpack mr-2"></i>
                        Inventory
                    </h5>
                    <div id="inventory-display" class="text-xs lg:text-sm text-gray-300 bg-gray-900 p-2 rounded min-h-8 lg:min-h-12 border border-gray-700">
